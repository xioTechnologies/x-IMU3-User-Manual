...
The result response is sent from the device to the host in response to the result command.  The key is \enquote{result} and the value is a \ac{JSON} object containing multiple key/value pairs.  Each key/value pair indicates the result of a component test.  Each key is the component name and the value is the string "Passed" if the test was passed.  The \ac{JSON} object will be empty if the self-test is in progress or if a self-test has not been performed.

\begin{table}[H]
    \begin{tabular}{l l l}
        \textbf{Example}*\textbf{:} & \texttt{\{} \\
        & \texttt{~~"result":~\{} & \\
        & \texttt{~~~~"Component A":} & \texttt{"Passed",} \\
        & \texttt{~~~~"Component B":} & \texttt{"Passed",} \\
        & \texttt{~~~~"Component C":} & \texttt{"Passed"} \\
        & \texttt{~~\}} \\
        & \texttt{\}\textbackslash r\textbackslash n} \\
    \end{tabular} \\
    \begin{tabular}{l}
