...
% The colour command is sent to the device to set the \ac{LED} colour.  The key is \enquote{colour} or \enquote{color} and the value is either an \ac{RGB} hex triplet expressed as a string, or null.
% %
% Setting the colour will override the normal \ac{LED} behaviour.
% %
% A value of null will restore the normal behaviour.
% %
...
The format command is sent to the device to format the SD card.  The key is \enquote{format} and the value is null.  Formatting will erase all data on the SD card.

\commandMessageExample{"format":null}

\subsubsection{Self-test command}

The self-test command is sent to the device to perform a self-test.  The key is \enquote{test} and the value is null.  The result of the self-test is obtained using the result command.  The device must be stationary during the self-test.

\commandMessageExample{"test":null}

\subsubsection{Result command}

The result command is sent to the device to obtain the result of the most recent self-test.  The key is \enquote{result} and the value is null.  The device will acknowledge a result command by sending a result response to the host.

\commandMessageExample{"result":null}

\subsubsection{Result response}

The result response is sent from the device to the host in response to the result command.  The key is \enquote{result} and the value is a \ac{JSON} object containing multiple key/value pairs.  Each key/value pair indicates the result of a test.  Each key is the test name and the value is the string "Passed" if the test was passed.  The \ac{JSON} object will be empty if the self-test is in progress or if a self-test has not been performed.

\begin{table}[H]
    \begin{tabular}{l l l}
        \textbf{Example}*\textbf{:} & \texttt{\{}\\
        & \texttt{~~"result":~\{} &\\
        & \texttt{~~~~"Test A":} & \texttt{"Passed",}\\
        & \texttt{~~~~"Test B":} & \texttt{"Passed",}\\
        & \texttt{~~~~"Test C":} & \texttt{"Passed"}\\
        & \texttt{~~\}}\\
        & \texttt{\}\textbackslash r\textbackslash n}\\
    \end{tabular}\\
    \begin{tabular}{l}
...
The erase command is sent to the device to erase the \ac{EEPROM}.  The key is \enquote{erase} and the value is null.  This command can only be used after the factory command.
